 ...
 *
 * `knownColumns` is the set of real top-level column names on the table. A key
 * that matches a known column but otherwise could be treated as a map access due
 * to special characters (often the case in materialized columns) will be quoted.
 */
export function toQuotedClickHouseKeyExpression(
  ...
  // Plain column or dotted JSON path: quote each bare segment that needs it,
  // leaving any already-backticked segment (produced by `mergePath`) alone.
  return expr
    .split('.')
    .map(segment =>
      segment.startsWith('`') && segment.endsWith('`')
        ? segment
        : quoteIdentifierIfNeeded(segment),
    )
    .join('.');
}
